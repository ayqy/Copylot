...
- 中文：`v4-11` 已把样本比较、回写、稳定性和收费前门槛融合成统一 verdict，当前结论仍是 `stay_validation`。对外只能写“当前优先验证方向”，不能写成“已经证明会收费”。
- EN: `v4-11` fuses comparison, writeback, stability, and the pre-payment gate into one verdict, and the current result is still `stay_validation`. External copy must stay in “currently prioritizing / validating” language and must not imply that monetization is already proven.
