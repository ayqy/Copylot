...
## 0.2) 20260712 `v4-13` 推送受当前运行环境限制
...
影响：
- `v4-13` 的代码、测试、证据、roadmap 与汇报已经全部落盘并通过统一测试入口，但当前无法在此环境内完成远端推送
...
- 在恢复网络访问后，直接推送现有提交 `440a073`，不需要重新生成代码或文档
...
## 9) 20260325-113320-growth 阻塞清单（PRD 门禁）

已观测阻塞（2026-03-25 11:33 CST）：
- `curl -I -L --max-time 12 https://copy.useai.online/` → `Could not resolve host: copy.useai.online`
- `curl -I -L --max-time 12 https://chromewebstore.google.com/` → `Could not resolve host: chromewebstore.google.com`
- `curl -I -L --max-time 12 https://1.1.1.1` → `Couldn't connect to server`
- 证据目录：`docs/evidence/growth/20260325-113320-growth/`
...
## 11) 20260325-115710-growth 阻塞清单（PRD 门禁）

已观测阻塞（2026-03-25 11:57 CST）：
- `curl -I -L --max-time 15 https://copy.useai.online/` → `Could not resolve host: copy.useai.online`
- `curl -I -L --max-time 15 https://chromewebstore.google.com/` → `Could not resolve host: chromewebstore.google.com`
- `curl -I -L --max-time 15 https://1.1.1.1` → `Couldn't connect to server`
- 证据目录：`docs/evidence/growth/20260325-115710-growth/`

影响：
- 命中 `network_blocked`，按 PRD 禁止所有 Playwright/MCP 浏览器自动化（外网与本地渲染均禁用）。
...
本轮已完成降级产物：
- 渠道文案：`docs/growth/assets/generated/20260325-115710-growth/channel-posts.md`
...
## 20260428-005403-growth
- 联系表单触达: 外网预检失败，跳过表单提交
- n8n webhook 适配: 缺少 GROWTH_N8N_WEBHOOK_URL
- listmonk API 适配: 缺少 GROWTH_LISTMONK_URL
- PostHog event 适配: 缺少 capture_url 或 api_key

## 20260428-005914-growth
