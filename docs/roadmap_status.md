...
## 当前进度
- 一句话结论：v1-47 已完成「发布阻塞解除 v2」可发布交付：`npm run publish:cws` 新增 `socks5://`/`socks5h://` 代理支持并保持既有 `HTTP(S)_PROXY/ALL_PROXY` 优先级；新增网络可达性预检（Preflight）与错误分类，输出可复制的 `Proxy Diagnostic Block + Preflight Report Block + Diagnostic Pack`，预检失败时给出最短修复建议且非 dry-run 会在 upload/publish 前阻断；并落盘用例/证据索引/简报（`docs/test-cases/v1-47.md`、`docs/evidence/v1-47/`、`docs/reports/v1-47-report.md`）。`bash scripts/test.sh` 于 2026-03-20 回归 PASS（扩展版本 1.1.22，产物 `plugin-1.1.22.zip`）。真实上架取证仍需人类提供可用代理/VPN（但已支持仅提供 socks5 代理形态）。
...
3. 转化入口与商店物料复核（Top3，收入优先）：在真实发布与商店可达后，按 `docs/publish/cws-preflight-checklist.md` 与既有 ASO 文档复核商店端入口/UTM/隐私口径一致性，并为下一轮“商店端转化”实验准备可复用对比口径与证据模板
