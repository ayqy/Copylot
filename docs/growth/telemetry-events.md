...
| `pro_entry_opened` | 打开 Pro 入口（升级 Pro / Pro Tab / Popup 意向直达卡片） | Popup：`src/popup/popup.ts` 点击 `#upgrade-pro-entry` 或 `.pro-intent-option`；Options：`src/options/options.ts` 激活 `#pro-tab` | `{ source: 'popup' \| 'options', campaign?: string }` | `campaign` 仅来源于用户输入（Options -> Pro Tab），不包含网页内容/复制内容/URL/标题 | `campaign` 为空不写入 props |
...
| `pro_waitlist_survey_copied` | 复制“付费意向问卷（可选）”成功 | Options：`src/options/options.ts` 点击 `#pro-waitlist-survey-copy` / `#pro-waitlist-survey-copy-open` 且写入剪贴板成功 | `{ source: 'popup' \| 'options', campaign?: string, prefill_used?: boolean, prefill_capability_count?: number, pay_*?, cap_*?, has_*? }` | 不包含任何网页内容/复制内容/URL/标题；复制内容为“环境信息 + 用户手动填写问卷”Markdown；`campaign` 仅来源于用户输入 | `prefill_*` 仅表示是否来自 Popup 预填直达，不记录用户原文；`campaign` 为空不写入 props |
