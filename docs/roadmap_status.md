...
## 当前进度
- 一句话结论：截至 2026-03-21，v1-58 已完成「按 campaign 合并导出 7d 获客效率（leads + distCopies + leadsPerDistCopy）」可发布交付：Options -> 隐私与可观测性 -> Pro 面板新增稳定入口 `#export-pro-acquisition-efficiency-by-campaign-7d-csv`（匿名 OFF 置灰且不读取/不推断 events；匿名 ON 一键下载 CSV）；合并口径复用 v1-54 `leads` 与 v1-57 `distCopies` 且可互证复算；证据落盘 `docs/evidence/v1-58/`；`bash scripts/test.sh` 回归 PASS。
...
3. 渠道分发/投放跑数与复盘固化 v4（Top3，收入优先，离线可推进）：用 v1-57 的“商店安装链接/完整投放包”做 1 轮渠道分发跑数；匿名 ON 后直接导出 v1-58「按 campaign 合并获客效率」7d CSV（`#export-pro-acquisition-efficiency-by-campaign-7d-csv`），并与 weekly digest（v1-50）/7d 明细（v1-51）互证；将合并 CSV 与截图索引按周归档，形成可持续的商业化增长证据资产（网络恢复后与商店真实跑数对齐）。
