...
5.  **【私人 Prompt 管理器】—— 打造您的专属 AI 工作流**
    ...
    -   **Copylot 解决方案：**
        ...
        -   所有 Prompt **通过浏览器账户云端同步**，您在任何一台电脑上配置的 Prompt 都会自动同步，无需重复设置。新建、修改或删除成功后，右键菜单、快捷位和网页内 Prompt 菜单会立即使用最新结果，不需要再点击同步按钮。
        -   若同步存储写入失败，编辑器会保留输入并明确提示失败；Popup 与 Chat 服务设置也会恢复到最后一次成功保存的状态，避免界面显示“已保存”但实际未生效。
        -   自动打开在线 AI 时会在复制成功反馈后约 **1 秒**跳转。内置服务按 DeepSeek、Kimi、豆包、通义、文心、GLM 等中国工具优先，其后再列出国外工具，并使用已核验的官方入口。
...
2. 在打开的“Prompt管理器”页面，您可以：
    ...
    - **可靠同步**：新建、编辑或删除 Prompt 后，右键菜单会按顺序安全重建并立即生效；并发使用 Prompt 时也不会覆盖刚保存的标题、模板或使用次数。
    - **中英文界面**：Prompt 编辑、预览、导入导出和 Chat 服务表单会随 Chrome 界面语言完整切换。
