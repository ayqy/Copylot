...
- [ ] 转化入口跑数基线：从商店（若可达）/或从 `plugin-*.zip` 安装回归 + 导出「Pro 意向漏斗摘要」与「证据包」并落盘截图索引（v1-40）
- [ ] WOM 小实验：对分享/评价/反馈入口做 1 轮低打扰实验，并用本地事件导出形成可量化对比口径

## 当前进度
- 一句话结论：v1-39 已完成 `publish:cws` 代理链路确定性修复 + 一键诊断 + 用例/简报落盘（可审计、可复现）；真实上架取证（Top1）仍需人类提供可达 Google API 的代理/VPN，在等待网络可达期间先做 v1-40「转化入口跑数基线」：用商店安装（若可达）或 `plugin-*.zip` 安装回归 + 导出 Pro 意向漏斗证据包并落盘，确保一旦发布即可立刻开始跑数与对比实验。
...
2. 转化入口跑数基线（Top2）：从商店（若可达）或 `plugin-*.zip` 安装当前可发布版本，核对 Popup/Options 转化入口；在隐私页导出「Pro 意向漏斗摘要」与「证据包」（v1-38）并落盘截图/证据索引，形成可量化对比口径（为后续 WOM/增长实验提供基线）
...
- 若 `chromewebstore.google.com` 亦不可达，则 Top2 的“从商店安装”同样需要代理/VPN；但 v1-40 将以 `plugin-*.zip` 安装回归作为不依赖外网的保底路径，确保基线证据仍可落盘。
