...
- 当前判断：`prds/v4-10.md` 已把当前领先路线整理成 `7d / 14d + campaign` 可直接导出的稳定性摘要，当前已经具备“三路线样本比较摘要 + 领先路线回写包 + 领先路线稳定性摘要 + A/B/C 门槛摘要”四段闭环；下一步不该再扩入口，而应转到统一融合判断，再复核是否真的接近收费评估。
...
- 进行中事项：
  ...
  - 已有三条验证路线、样本比较摘要、领先路线回写包、领先路线稳定性摘要与门槛摘要入口，但 `campaign` 支撑仍未完全收敛，仍需继续扩大真实样本，而不是把一次领先误判成长期需求。
  - 软件工厂入口仍未恢复稳定自动闭环；当前最后一项收费评估仍需在人工接管下推进。
- 当前经营判断：
  - 当前最小可交付增量已经把三条 Pro 验证路线的说明、分发素材、匿名归因口径、样本比较摘要、领先路线回写包、领先路线稳定性摘要和 A/B/C 门槛摘要接到一起；下一阶段应停止扩路线，转而产出统一融合判断，再决定是否进入收费评估。

## 下一步最重要的 3 件事（收入优先）

1. 把“路线样本比较摘要 + 领先路线回写包 + 领先路线稳定性摘要 + A/B/C 门槛摘要”融合成统一 verdict，明确当前是否仍应停在验证阶段。
2. 基于统一 verdict 补一份收费评估审计包，只做判断与证据，不做支付实现。
3. 继续收集跨 campaign 的真实任务样本，确认 `advanced_cleaning` 的领先不是 acquisition 偏差。
