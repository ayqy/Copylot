...
- [x] S3 Pro 路线验证
  ...
  - [x] 仅围绕批量采集与整理设计 Pro 路线说明与验证素材。
  - [x] 仅围绕结构化导出与下游工作流设计 Pro 路线说明与验证素材。
  - [x] 只有在激活、复用与口碑信号成立后，才评估支付与收款链路。（`v4-12` 已补收费评估审计包；当前仍只到评估与复核，不做支付实现。）

- [x] S4 收费评估前样本治理
      进度：3/3（100%）
  - [x] 把跨 campaign 的领先路线支撑/冲突做成可导出的复核包。
  - [x] 把 `stay_validation` 外部话术守门做成单独复核包。
  - [x] 把同一窗口内的三项检查固化成单独的人类批准门槛 tracker；未通过时继续 `hold_validation`。

- [ ] S4 跨 campaign 样本复核与增长纪律
      进度：4/5（80%）
  - [x] 把 `advanced_cleaning` 的跨 campaign 样本复核做成产品内可导出审计包，验证领先是否来自真实需求而非 acquisition 偏差。
  - [x] 把官网、商店、扩展内 Pro 说明与分发素材统一锁定为“当前优先验证方向 / stay_validation”，避免任何已验证收费或已上线支付的暗示。
  - [x] 固化“并行开始增长循环”的 campaign 口径、分发入口与证据导出，让新增安装、路线打开、素材复制和后续复核可以同口径对账。
  - [x] 先把人类批准 handoff 包做成单独导出物，显式汇总 blocker、批准问题、guardrail 与 no-implementation 边界。
  - [ ] 只有在跨 campaign 复核、收费评估审计包与 tracker 同窗连续放行后，才把 handoff 升级为收费实现规划。
