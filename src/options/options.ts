...
async function loadSettings() {
  try {
    ...
    renderAnonymousUsageDataStatus(Boolean(currentSettings.isAnonymousUsageDataEnabled));
...
function renderAnonymousUsageDataStatus(enabled: boolean): void {
  const status = document.getElementById('anonymous-usage-data-status');
  if (!status) return;
  status.dataset.state = enabled ? 'enabled' : 'disabled';
  status.textContent = getMessage(
    enabled ? 'localUsageRecordsStatusOn' : 'localUsageRecordsStatusOff'
  );
}
  ...
  try {
    ...
    if (!enabled) {
      try {
        ...
        console.error('Failed to clear local usage records:', error);
        showNotification(getMessage('localUsageRecordsClearFailed'), 'error');
      }
    }

    renderAnonymousUsageDataStatus(enabled);
  } catch (error) {
    console.error('Failed to update local usage records:', error);
    elements.anonymousUsageDataSwitch.checked = previousValue;
    renderAnonymousUsageDataStatus(previousValue);
...
function setupEventListeners() {
  ...
  if (['prompts', 'chat-services', 'privacy', 'pro'].includes(requestedTab)) {
