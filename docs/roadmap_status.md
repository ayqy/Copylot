...
## 当前进度
- 一句话结论：v1-43 已完成「WOM 小实验」可发布交付：统一 Popup/Options/评分引导 的 Chrome Web Store UTM（`utm_source=copylot-ext`、`utm_campaign=v1-43`、`utm_medium=popup|options|rating_prompt`），并在 Options -> 隐私与可观测性新增「WOM 摘要」与「WOM Evidence Pack」导出（字段白名单、匿名开关关闭时不记录/不导出）；证据资产落盘于 `docs/evidence/v1-43/`（含 telemetry off 对照 + 截图索引）。真实上架取证仍受网络可达性阻塞（待可用代理/VPN）。
...
2. 真实跑数取证与对比（Top2）：真实发布后从商店安装当前版本，按 `docs/evidence/v1-42/index.md` 与 `docs/evidence/v1-43/index.md` 同口径再次导出「Pro 意向漏斗摘要/证据包」与「WOM 摘要/证据包」并落盘截图索引（用于与 zip 回归基线对比：source 拆分计数/转化率 + UTM 可核验）
3. 基于 v1-43 的 WOM 量化口径做下一轮收入导向实验（Top3）：在不增加打扰/不新增权限前提下，优先选择“能提高商店访问/评价点击/反馈点击”的最小文案或触发阈值调整，并形成 v1-44 子 PRD（含用例/断言/证据落盘）
